...
function screenSetup() {
    ...
    <div class="grid" style="gap:12px">
      ...
      <div class="row">
        ...
        <button id="addPlayer" class="accent">Add</button>
      </div>
...
function screenDiscuss(){
  ...
  el.innerHTML = `
    <div class="card">
      <h2 class="center" style="margin:0 0 6px">Discussion Phase</h2>
      <p class="small center">Talk it out. Then each player votes.</p>
      <div class="row" style="justify-content:center; margin-top:8px">
        <button id="tallyBtn" class="full">Tally Votes</button>
      </div>
    </div>
    <div class="card">
  ...
  state.players.forEach((p, idx)=>{
    ...
    item.querySelector('button').onclick = (e)=>{
      const i = +e.currentTarget.dataset.i;
      state.players[i].votes = (state.players[i].votes||0) + 1;
      save();
      render();
    };
  ...
  el.querySelector('#tallyBtn').onclick = ()=>{
    // find highest voted
    let maxVotes = -1;
    let eliminated = null;
    state.players.forEach(p=>{ if((p.votes||0) > maxVotes){ maxVotes = p.votes; eliminated = p; } });
    if(eliminated){
      alert(`${eliminated.name} (${eliminated.role}) is eliminated!`);
      state.players = state.players.filter(p=>p!==eliminated);
    }
    state.players.forEach(p=> delete p.votes);
